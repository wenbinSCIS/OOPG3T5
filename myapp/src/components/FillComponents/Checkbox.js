...
function Checkbox({
  ...
  function handleTextinChange(parentName, parentCheckboxValue, e) {
    ...
    const selectedItemIndex = newSelectedItems.findIndex(
      (item) =>
        item.name === parentCheckboxValue && item.type === "Checkbox-text"
    );
    ...
    <div style={{ position: "relative" }}>
      ...
      <div className={number}>
        ...
        {options.map((option, index) =>
          option.optionType == "Checkbox-text" ? (
            <div style={{ display: "flex", alignItems: "center" }}>
              <Form.Check
                label={option.optionValue}
                checked={
                  data != null &&
                  data.find((item) => item.name === option.optionValue)
                    ? true
                    : false
                }
                name={name}
